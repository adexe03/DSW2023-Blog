                ...
                <div class="p-6 text-gray-900">
                    ...
                    <div class="grid grid-cols-6">
                        <div class="col-start-1 col-span-1 m-3">
                            <a href="{{ route('posts.index') }}" class="p-3 bg-blue-200 hover:bg-blue-500">Volver</a>
                        </div>
                        @if (auth()->user()->id == $post->user_id )
                        <div class="col-start-5 col-span-1 text-right m-3">
                            <a href="{{ route('posts.edit', $post) }}" class="p-3 bg-green-200 hover:bg-green-500">Editar</a>
                        </div>
                        <div class="col-start-6 col-span-1 text-right m-0 p-0">
                            <form action="{{ route('posts.destroy', $post) }}" method="POST" class="m-0 p-0">
                                @csrf
                                @method('DELETE')
                                <input type="submit" value="Borrar" class="bg-red-200 hover:bg-red-500 p-3" />
                            </form>
                        </div>
                        @endif
                    </div>
